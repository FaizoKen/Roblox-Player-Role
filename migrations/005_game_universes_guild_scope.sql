...
-- relied on universe_id being unique. Drop the FK FIRST so the PK can be
-- dropped below. Integrity is now enforced at the application layer (ingest
-- writes only when (universe_id, secret) match a registered row). Cascade-
-- on-delete is replaced by an explicit DELETE in the delete_universe handler.
