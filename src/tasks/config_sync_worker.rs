//! Debounces ConfigSyncEvents (5s) so rapid edits coalesce before kicking off a
//! 100k+ member rebuild.
...
pub async fn run(mut rx: mpsc::Receiver<ConfigSyncEvent>, state: Arc<AppState>) {
    ...
    loop {
        ...
        for key in ready {
            ...
            tracing::debug!(guild_id, role_id, "Bulk sync (debounced)");
            if let Err(e) = sync::sync_for_role_link(guild_id, role_id, &state).await {
                tracing::error!(guild_id, role_id, "Bulk sync failed: {e}");
            }
        }
    }
