...
<body>
    ...
    <div id="linked-section" class="card hidden">
        ...
        <p style="margin-top:14px; font-size:13px; color:#a1a7ad;">
            Receiving Roblox roles in servers you didn't intend?
            <a href="/auth/my_servers?from=/roblox-player-role/verify" style="color:#00a2ff;">Choose which servers receive roles &rarr;</a>
        </p>
        <hr class="divider">
        <div class="actions">
    ...
    <script>
    const API = '{base_url}';
    const PLUGIN_SLUG = 'roblox-player-role';
    ...
    (function patchLoginHref() {{
        ...
        if (!link) return;
        const returnTo = '/roblox-player-role/verify?guild=' + encodeURIComponent(guildId);
    ...
    async function applyGuildContext() {{
        ...
        if (wasDisabled) {{
            showGuildCtx(isLinked
                ? 'Enabled Roblox roles for ' + nameHtml + ' — roles apply on the next sync.'
                : 'Enabled Roblox roles for ' + nameHtml + ' — finish linking below to receive roles.');
        }} else {{
            showGuildCtx(isLinked
                ? 'Roblox roles are active in ' + nameHtml + '.'
                : 'Once linked, Roblox roles will apply in ' + nameHtml + '.');
        }}
    }}
