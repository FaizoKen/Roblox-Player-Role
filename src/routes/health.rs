...
pub async fn studio_plugin_rbxm() -> impl IntoResponse {
    (
        [
            (header::CONTENT_TYPE, "application/octet-stream"),
            (
                header::CONTENT_DISPOSITION,
                "attachment; filename=\"Roblox-Player-Role.rbxm\"",
            ),
            (header::CACHE_CONTROL, "public, max-age=3600"),
        ],
        include_bytes!("../../studio-plugin/dist/Roblox-Player-Role.rbxm").as_slice(),
    )
}
...
pub async fn health(State(state): State<Arc<AppState>>) -> Json<Value> {
    ...
    let svc_fut = check_service(&state.http, "Roblox Web API", "https://users.roblox.com/v1/users/1");
